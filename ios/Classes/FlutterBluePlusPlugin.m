...
- (void)handleMethodCall:(FlutterMethodCall *)call result:(FlutterResult)result
{
    @try
    {
        ...
        else if ([@"readCharacteristic" isEqualToString:call.method])
        {
            ...
            NSUInteger *characteristicIndex = [args[@"characteristic_index"] unsignedIntegerValue];
            NSString  *serviceUuid          = args[@"service_uuid"];
            NSUInteger *serviceIndex        = [args[@"service_index"] unsignedIntegerValue];
            NSString  *secondaryServiceUuid = args[@"secondary_service_uuid"];
            NSUInteger *secondaryServiceIndex = [args[@"secondary_service_index"] unsignedIntegerValue];
...
- (CBCharacteristic *)locateCharacteristic:(NSString *)characteristicId 
                       characteristicIndex:(NSUInteger *)characteristicIndex
                                peripheral:(CBPeripheral *)peripheral
                                 serviceId:(NSString *)serviceId 
                              serviceIndex:(NSUInteger *)serviceIndex
                        secondaryServiceId:(NSString *)secondaryServiceId 
                     secondaryServiceIndex:(NSUInteger *)secondaryServiceIndex
                                     error:(NSError **)error
{
    // primary
    CBService *primaryService = [self getServiceFromArray:serviceId index:serviceIndex array:[peripheral services]];
    ...
    {
        secondaryService = [self getServiceFromArray:secondaryServiceId index:secondaryServiceIndex array:[primaryService includedServices]];
    ...
    // characteristic
    CBCharacteristic *characteristic = [self getCharacteristicFromArray:characteristicId index:characteristicIndex array:[service characteristics]];
    if (characteristic == nil)
    {
...
- (void)peripheralIsReadyToSendWriteWithoutResponse:(CBPeripheral *)peripheral
{
    ...
    NSUInteger  *characteristicIndex = [request[@"characteristic_index"] unsignedIntegerValue];
    NSString  *serviceUuid          = request[@"service_uuid"];
    NSUInteger  *serviceIndex       = [request[@"service_index"] unsignedIntegerValue];
    NSString  *secondaryServiceUuid = request[@"secondary_service_uuid"];
    NSUInteger  *secondaryServiceIndex = [request[@"secondary_service_index"] unsignedIntegerValue];
